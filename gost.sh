...
function Install_ct()
{
    ...
    `wget --no-check-certificate https://raw.githubusercontent.com/cntzjjs/EasyGost/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system`
    `mkdir /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/cntzjjs/EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost`
...
function read_d_ip()
{
    ...
    echo -e "    具体IP地址的填写, 取决于接收该流量的服务正在监听的IP(详见: https://github.com/cntzjjs/EasyGost)"
    read -p "请输入: " flag_c
}
